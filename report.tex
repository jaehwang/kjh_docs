...
Caelum의 Guilherme Silveira이 진행한 튜토리얼.
...
하더니만 제목과는 다르게 막바로 Restfulie\footnote{http://restfulie.caelum.com.br/}라는 REST framework에 대한 설명에 
돌입했다. 어쩔 수 없이 wikipedia를 참고했다. REST는 ``WWW과 같은 분산 
하이퍼미디어 시스템을 위한 소프트웨어 아키텍처의 한 
형식(architectural style)''\footnote{http://ko.wikipedia.org/wiki/REST}이라고 
한다.
...
항공 예약 사이트(TICKET), 신용카드 결재 사이트(PAY), 캘린더 사이트(CAL)가 
있다고 하자.
...
자동화를 엄청나게 강조했다. 불행히도 울분을 토로하는 연극 배우 같이
``연기'' 형식으로 이야기를 하는 바람에 자세한 내용을 알아들을 수는 없었다.
...
Prototyping이 ``이걸 만들 수 있나?''에 대한 답을 찾는 것이라면, pretotyping은
``사람들이 이것을 좋아할까?''에 대한 답을 찾는 것이라고 한다.
...
\textit{Programming Clojure}의 저자 Stuart Halloway가 발표를 했다. 
내가 Con에서 본 발표자 중에 가장 명쾌하게 발표를했다고 생각한다. 
...
11월 3일에는 챔피언 퍼레이드가 열렸다. 컨퍼런스 조전  스케줄이 조정이 
되어 퍼레이드를 볼 수 있는 40분의 시간이
...
그래도 조금 정리하자면 fear를 다스리지 못하면 gantt chart라는 재앙이 온다는 
얘기다.
...
인터넷을 좀 검색하니 이 talk의 주요 내용을 다룬 기사가 있었다.
\texttt{http://www.infoq.com/articles/better-best-practices}.
 
\subsection{Conference 11/3 Dataflow Programming: a Scalable
  Data-Centric Approach to Parallelism}
 
발표자는 Prevasive사의 Jim Falgout.
...
Scrum 코치로 활동하고 있는 WIZEWERX의 Mike Sutton전한 동화같은 이야기.
...
저녁에 구글 리셉션이 있었다. 조잡한 기념품과 미국인들은
