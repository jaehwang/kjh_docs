...
소프트웨어 개발 infra를 구축하다보면 어쩔수 없이 웹 프로그래밍을 하게 되는 
경우가 많다. 게다가 세상이 이미 웹이라는 거대한 망으로 묶여 있기도 해서 
WWW 관련 기술에는 관심을 안 가질 수 없다고 생각한다.

REST(Representational State Transfer)에 대해서는 다 알고 왔을 거라고 전제를 
하더니만 제목과는 다르게 막바로 Restfulie라는 REST framework에 대한 설명에 
돌입했다. 어쩔 수 없이 wikipedia를 참고해보니 REST는 ``WWW과 같은 분산 
